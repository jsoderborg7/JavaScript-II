...
// Problem 1
// Find out what company each participant works for

let whatCompany= [];
 runners.forEach(item => whatCompany.push(`${item.first_name} ${item.last_name} ${'works at'} ${item.company_name}`));
 console.log(whatCompany);

 // Problem 2
//  Find who donated over $100

let bigSpenders= [];
bigSpenders= runners.filter(item => item.donation >= 100);
console.log(bigSpenders);

// Problem 3
// Make the last names appear in all caps for trophies

let trophyNames = [];
trophyNames= runners.map(item => item.last_name.toUpperCase());
console.log(trophyNames); 